...
const run = () => {
  ...
  storage.get(['binding'], obj => {
    ...
    console.log(binding);
    window.Mousetrap.bind(binding, e => {
      const active = document.activeElement;
      const tag = active.tagName.toLowerCase();
      if (tag === "textarea" || tag === "input" || active.contentEditable === "true") return;
      if (isInverted) uninvertsave();
      else invertsave();
    });
  });
};
